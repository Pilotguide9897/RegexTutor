...
This tutorial will explore in-depth some common components of regular expressions and their application in an expression designed to match URLs. By the end, you will hopefully have a greater understanding of 
how regular expressions work and the ways in which you can use them yourself to target and match text.

## Summary

With the introduction out of the way, today we will be exploring the following regular expression for identifying URLs in text:

`/^(https?:\/\/)?([\da-z\.-]+)\.([a-z\.]{2,6})([\/\w \.-]*)*\/?$/`

This regular expression relies on a combination of grouping, character classes, quantifiers, and anchors to match URLs. More specifically, it matches for strings which start with an optional "http://" or "https://" protocol, followed by a domain name consisting of one or more letters, digits, dots, and hyphens, then proceeds to search for a top-level domain of two to six letters or dots (e.g., .com, .org, .co, .uk, .gov, .edu) before finally attempting to match any optional path or query parameters that follow the domain name.

| HTTP Protocol    | Domain          | Top-level Domain   | Optional path/Query Parameters |
| ---------------- | --------------- | ------------------ | ------------------------------ |
| `(https?:\/\/)?` | `([\da-z\.-]+)` | `\.([a-z\.]{2,6})` | `([\/\w \.-]*)*\/?$`           |

For the purpose of organization, this tutorial will break down the regular expression into its individual components and examine what each type does, including anchors, quantifiers, character classes, grouping and capturing, as well as look-ahead/look-behind assertions.
...
Regular expressions are a powerful and robust tool for pattern matching and text manipulation. They offer a diverse range of features and options that can be used to match complex patterns and extract specific information from text. Not all regular expressions necessarily need to take advantage of all these features, however. In fact, many simple regular expressions can be written using just a few basic elements, such as character classes, quantifiers, and anchors. Sometimes, a straightforward regular expression can be more effective than a complex one that uses every available feature. Still, as a software developer it is important to be fully aware of the range of regular expression features and how they can be used to solve different pattern matching problems. Knowledge of how to use back-references, look-ahead/behind assertions, as well as other advanced features can make it easier to write efficient and effective regular expressions for more complex pattern matching tasks. Overall, the key to using regular expressions effectively understanding the problem that one is trying to solve, and leveraging the appropriate regular expression features to match the patterns and extract the information you need.
...
For the purpose of this tutorial and to provide a more complete overview of regular expressions in general, brief sections will still be dedicated to these components despite their disuse in this particular example. Further, please note that although they are not necessarily a part of the regular expression syntax itself, the (`/`) characters used in the example above at the beginning and end of the regular expression are used to indicate the beginning and end of the regular expression pattern. Specifically, when a regular expression is written in JavaScript, the (`/`) indicates to the interpreter that the enclosed text is a regular expression and serves to separate the regular expression from any flags that may modify its behaviour (e.g., (`/my regular expression/i`), where (`i`) is a flag that renders the regular expression case insensitive).

## Part-by-Part-Breakdown

Before analyzing each regular expression component individually, it may be helpful to proceed through the expression step-by-step from left to right and analyze each part to gain a better understanding of the information that is to follow further below.
...
3. (`(https?:\/\/)?`): This section represents a capturing group, denoted by the parentheses (). The (`?`) after the group makes it optional.
...
- (`a-z`): Matches any lowercase letter (a-z).
- (`\.`): Matches a literal period ".". The backslash is an escape character.
- (`{2,6}`): This is a quantifier that specifies the preceding character set must match at least twice and a maximum of six times.

7. (`([\/\w \.-]\*)`): Yet another capturing group.
...
9. (`\/?`): This section of the expression matches an optional forward slash "/". The backslash "\" is an escape character, and the question mark "?" indicates that the preceding forward slash character is optional.

10. (`$`): This symbol indicates the end of the line. The pattern must match until the end of the input.

11. (`/)`: The forward slash at the end is a delimiter that signifies the end of the regular expression pattern. It is the partner to the opening delimiter. Together with the opening forward slash, it encapsulates the entire regular epression.

### Anchors

`/^(https?:\/\/)?([\da-z\.-]+)\.([a-z\.]{2,6})([\/\w \.-]*)*\/?$/`

Anchors are a category of special character used in regular expressions that allow users to match text from specific *positions* within a string rather than targeting specific characters. The two most commonly used anchors are the caret (`^`) and dollar sign `$`. Notably, these anchors are used in our URL matching regular expression. Let us explore how they are used.

1.  The caret (`^`) anchor is used to match the beginning of a line (or an entire string in single-line mode). In this example it is used to indicate the start of our url.
2.  The dollar sign (`$`) anchor is used to match the end of a line or string. In this instance, it is used to indicate the end of our URL. Note that though not used for this purpose in the regular expression above, (`$`) is also sometimes used for back-references in the replacement part of a regular expression operation. More information on its use in back-references is provided below in the section on [back-references](#back-references).

Although they are not used in this particular case, additional examples of anchors include (`\b`) and (`\A`), which search for word boundaries and for the beginning of strings, respectively. To put it another way, (`b`) lets one perform whole-word searches using regular expressions in the form of (`\bmyWord\b`) (e.g., (`\bapple\b`) can be used to match "apple" as a stand alone word in text, but would fail to match if apple is part of another word such as is the case with "pineapple"). Using this anchor at the beginning or end of a word also allows users to specify that a word must have a boundary before or following a certain pattern (e.g., to find words that end with "ing", you can use the pattern (`\w+ing\b`). This will match words like "running", "jumping", but not "bring").  Alternatively, (`\B\`) is the negated version of (`\b`) and allow for selection of any position that is *not* a word boundary. For instance, to match the letter "a" only if it is not at a word boundary, you can use (`\Ba\B`). This will match the "a" in "banana", but not in "apple".

Whereas (`\b`) searches for word boundaries, (`\A`) detects the beginning of strings of text. In this regard, (`\A`) functions quite similarly to the caret (`^`), discussed above, with a slight nuance differentiating how each functions. Specifically, (`\A`) matches the start of the *entire* string no matter what and ensures that the pattern that follows must start at the very beginning of the entire string. The caret (`^`) on the other hand, usually matches the start of a line, but can also match the start of the entire string. By default, (`^`) matches the start of a line in multiline mode. In regular mode, however, it matches the start of the entire string. Notably, some regex flavors allow modifying (`^`) to always match the start of the entire string regardless of multiline mode.
...
Why don't we break these bracket expressions down in more detail? First up is, (`[\da-z\.-]`). This bracket expression uses (`\d`) to match any digit from 0 to 9. Additionally, its use of (`a-z`) allows it to match any lowercase letter. Finally, (`\. -`) escapes and matches a dot (`.`), a hyphen (`-`), and a blank space. Together, these smaller portions allow this expression to match any alphanumeric characters, dots, hyphens, and spaces that might be used in a URL. Moving on, next up we have (`[a-z\.]`). In this bracket expression, like before,(`a-z`) allows matching of any lowercase letter and (`\.`) escapes and matches the literal dot character. Together, these patterns allow this expression to match lowercase letters and dots for matching top-level domain names (e.g., .com, .edu). Last but not least we have (`[\/\w \.-]`). This bracket expression first uses (`\/`) to escape and match literal forward slashes (`/`). Next, it uses the predefined `\w` character class (note how character classes and bracket expressions can be used synergystically) to match any word character. Then it leaves a blank space to match any empty spaces, before finally using (`\. -`) in a fashion similar to bracket expression one to  match a dot (`.`), a hyphen (`-`), and a blank space. Overall, this bracket expression allows matching of forward slashes, word characters, spaces, dots, and hyphens to target the wide variety of additional URL query parameters. 

Generally speaning, when attempting to match a set of characters that follow a specific pattern or order, bracket expressions may be a better choice, as they provide a greater degree of flexibility, though overall, the choice between character classes and bracket expressions depends on the specific requirements of your pattern, and often a combination of both may provide the best solution. With this example now in place, it is my hope that you can see how, overall, character classes can be an invaluable tool for writing more concise and powerful regular expressions.
...
Look-ahead and look-behind, collectively referred to as 'lookaround' allow matching of text based on what comes before and after a specific pattern, specifically without including the text that comes before and after the text in the final match, respectively. Critically, the reason to use lookarounds rather than merely specifying that something should be present in your string is when you would like to match a pattern *only* if it is followed or preceded by another specific pattern, and you do not want to include the other pattern in the match. Lookarounds are considered *zero-width assertions*, meaning that they do not consume characters in a string and merely asserting whether a match is possible.
 ...
 Concerning our regular expression for detecting URLs, as noted above, this expression does not take advantage of lookarounds. Regardless, for the sake of being thorough let us briefly discuss the two categoreis of lookaround in slightly expanded detail.
